...
case "$1" in
    exorcise)
        ...
        exec /opt/miniforge3/envs/exorcise/bin/Rscript /exorcise/bin/exorcise.R "${@:2}"
        ;;
    ntByCycle)
        echo "Running exorcise:ntByCycle"
        exec /opt/miniforge3/envs/exorcise/bin/Rscript /exorcise/bin/ntByCycle.R "${@:2}"
        ;;
    count_reads)
        echo "Running crispr_tools:count_reads"
        exec /opt/miniforge3/envs/crispr_tools/bin/count_reads.py "${@:2}"
        ;;
    crispr_pipeline)
        echo "Running crispr_tools:crispr_pipeline"
        exec /opt/miniforge3/envs/crispr_tools/bin/crispr_pipeline.py "${@:2}"
        ;;
