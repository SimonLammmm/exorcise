"""DrugZ: identification of chemogenomic interactions from CRISPR screens.

Vendored from crispr_tools, which vendored it from
https://github.com/hart-lab/drugz (Medina Colic / Traver Hart, MIT licence).

The numerical routines are reproduced unchanged so that results remain
comparable with published DrugZ output. Only the standalone argparse CLI has
been removed: DrugZ is driven from exorcise_screens.pipeline, which calls
drugZ_analysis() in process rather than shelling out.
"""
...
# DrugZ assigns into slices of borrowed frames throughout; the warning is noise.
pd.options.mode.chained_assignment = None

#: Read depth every sample is normalised to before fold changes are taken.
norm_value = 1e7
...
def drugZ_analysis(args):
    ...
    if args.unpaired or (len(control_samples) != len(treatment_samples)):
        ...
        per_gene_scores2 = pd.DataFrame(
            fold_change2.groupby('GENE')['zscore'].apply(lambda x: pd.Series([np.nansum(x.values),
